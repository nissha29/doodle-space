...
export const getDrawable = (shape: Shape, generator: RoughGenerator) => {
  switch (shape.type) {
    case "rectangle": {
      ...
      return generator.rectangle(x, y, width, height, { stroke: 'white', roughness: 1.5, seed });
    }
    case "circle": {
      const { x, y, diameter, seed } = shape;
      return generator.circle(x, y, diameter, { stroke: 'white', roughness: 1.5, seed });
    }
    case "diamond": {
      const { diamondPoints, seed } = shape;
      return generator.polygon(diamondPoints, { stroke: 'white', roughness: 1.5, seed });
    }
    case "arrow": {
      const { x1, x2, y1, y2 } = shape.shaft;
      const { tip, left, right, seed } = shape;
      const shaft = generator.line(x1, y1, x2, y2, { stroke: 'white', roughness: 1.5, seed });
    ...
    case "line": {
      ...
      return generator.line(x1, y1, x2, y2, { stroke: 'white', roughness: 1.5, seed });
    }
    default:
